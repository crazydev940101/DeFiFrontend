...
const TokenCardPanel = (props) => {
    ...
    const testCallback = useCallback(data => {
        const sample = {
            token_address: '0xf2E8480FA925a34f8a5F82D3Edb52BB6999fc37E',
            user_id: '6471031307ddd921a87a6c07',
            owner_address: '0x8a6e0BeE2bb07fF5c8559817e12F92F5c19D0ea0'
        }

        const updateData = [...itemList];
        updateData.unshift({ ...sample, isLoading: true })
        itemListRef.current = updateData;
        setItemList(itemListRef.current);
    })

    const newTokenBuyingCallback = useCallback(data => {
        const updateData = [...itemList];
            ...
            if (!data.exist) {
                const tempData = itemListRef.current.map(item => {
                    if (item?.token_address?.toLowerCase() === newData?.token_address?.toLowerCase()) {
                        ...
                        props.sendNewTokenBought({ token_address: newData.token_address });
                        return update;
                    } else {
                        return item;
                    }
    ...
    useEffect(() => {
        if (socket) {
            ...
            socket.on("snipper:buying", newTokenBuyingCallback);
            socket.on("snipper:bought", newTokenBoughtCallback);
            // socket.on("test:blocknumber", testCallback);
            return () => {
                if (socket) {
                    socket.off("token:rugpool", rugPoolCallback);
                    socket.off("snipper:buying", newTokenBuyingCallback);
                    socket.off("snipper:bought", newTokenBoughtCallback);
                    // socket.off("test:blocknumber", testCallback);
                }
            };
        }
    }, [socket, itemList]);
                        ...
                        <Card key={key} style={{ backgroundColor: key % 2 === 0 ? globalVariables.BACKGROUND_COLOR : '#ffffff' }}>
                            ...
                            <CardBody>
                                ...
                                <Row>
                                    <Col className="pr-md-1" md="6">
                                        <Row>
                                            <Col className="pr-md-1" md="2"><CardTitle>Owner Address</CardTitle></Col>
                                            <Col className="pr-md-1" md="6"><CardText>{ele?.owner_address}</CardText></Col>
                                        </Row>
                                    </Col>
                                </Row>
                                <Row>
