...
const AddSniperPanel = (props) => {
    ...
    useEffect(() => {
        if (props.tokenDataToAdd) {
            const updateItem = { ...newItem };
            updateItem.token_address = props.tokenDataToAdd.token_address;
            setNewItem(updateItem);
            showModalFlag(true);
        }
    }, [props.tokenDataToAdd])
    ...
    const onAddToSniper = async () => {
        ...
        if (newItem.func_regex === "") {
            showNotify('Please input function.', 'danger');
            return;
        }
