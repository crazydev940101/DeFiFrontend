...
const TokenList = (props) => {
    ...
    useEffect(() => {
        ...
        if (index >= 0) {
            ...
            jsonArray.unshift(responseData);
            setItemList(jsonArray);
        } else {
            itemList.unshift(responseData);
                ...
                if (response.status === 200 && isMounted()) {
                    if (response.data.pendingLists) {
                        const updatePending = response.data.pendingLists.map(pending => {
                            ...
                            return { ...pending, slippage: 10, estimationPrice: 0, percentage: 100, utils: utils, is_rug: false, isApproveDisabled: pending.approved, isSellDisabled: false, socket_message: '' }
                        });
                        itemListRef.current = updatePending;
                        setItemList(itemListRef.current);

                        /** init background color % 2 */
                        onChangeBackgroundColor(response.data.pendingLists);
            ...
            if (!data.exist) {
                ...
                const tempData = itemListRef.current.map(item => {
                    if (item?.token_address?.toLowerCase() === newData?.token_address?.toLowerCase()) {
                        ...
                        const update = { ...item, ...newData, utils: utils, slippage: 10, estimationPrice: 0, percentage: 100, isApproveDisabled: false, isSellDisabled: false, is_rug: false, socket_message: '', isLoading: false, exist: false };
                        return update
                ...
                if (tempData) {
                    ...
                    itemListRef.current = finalData;
                    setItemList(itemListRef.current);
                    console.log(1005, finalData);
                }
    ...
    /** init background color % 2 */
    const onChangeBackgroundColor = (list) => {
        const background_color_list = list.map((item, key) => {
            if (key % 2 == 1)
                return '#ffffff';
            else
                return '#f0f0f0';
        });
        setBackgroundColor(background_color_list);
    }
        ...
        <>
            {loadingAllData ? (
                ...
                itemList.map((ele, key) => (
                    ...
                    ele.isLoading ? (
                        <Card key={key} style={{ backgroundColor: backgroundColor[key] }}>
                            <CardHeader>
                                <Row>
                                            ...
                                            <Col className="pr-md-1" md="3">
                                                {
                                                    ele.utils?.loading_price_button ?
                                                        (<ClipLoader color={globalVariables.GREEN} size="50px" />
                                                            ...
                                                            <Button color="btn1" className="btn1" onClick={(e) => onEstimatePrice(e, key, -1, itemList, setItemList, props, showNotify, ApiCall, apiConfig, 1,)}>
                                                                Detect Price
                                                            </Button>
                                                        )
                                                }
                                            </Col>
                                            <Col className="pr-md-1" md="4"><CardTitle style={{ float: "left", fontSize: "16px" }}>{ele ? ele.estimationPrice : 0} &nbsp; {globalVariables.BASE_TOKEN_SYMBOL}</CardTitle></Col>
                                        </Row>
                                        ...
                                        <Row>
                                            ...
                                            <Col className="pr-md-1 toggle_botton" md="2">
                                                <ToggleButton
                                                    ...
                                                    onToggle={(value) => onDetectRugPool(value, key, -1, itemList, setItemList, props, ApiCall, apiConfig, 1)} />
                                                *Detect Rug Pull
                                            </Col>
                                            {ele.is_rug === 0 ? (
                                                <Col>
                                                    <CardText title="Detected rug pull!!!" className="css_rug_pool_state_false"><i className="tim-icons icon-alert-circle-exc" /><em style={{ position: 'absolute' }}>{ele.socket_message}</em></CardText>
                                                </Col>
                                            ) : (
                                                <Col>
                                                    <CardText title="No detected rug pull" className="css_rug_pool_state_true"><i className="tim-icons icon-alert-circle-exc" /><em style={{ position: 'absolute' }}>{ele.socket_message}</em></CardText>
                                                </Col>
                                            )
                                            }
                                        </Row>
