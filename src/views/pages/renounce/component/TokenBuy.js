            ...
            try {
                ...
                if (response.status === 200) {
                    if (isMounted()) {
                        ...
                        if(response.data.is_start === null) setDisabled(false);
                        else setDisabled(response.data.is_start);
    ...
    const onAddHandler = async () => {
        ...
        try {
            ...
            if (response.status === 200 && response.data.data) {
                ...
                const responseData = { ...response.data.data, slippage: 50, estimationPrice: 0, percentage: 100, utils: utils, isApproveDisabled: false, isSellDisabled: false, is_rug: false, socket_message: '', isLoading: false, exist: false }
    ...
    const onSetDefault = async () =>  {
        ...
        try {
            ...
            if (response.status === 200 && response.data) {
                console.log(2, response.data.autoSetting);
                const responseData = response.data.autoSetting;
                // setNewItem(response.data.autoSetting);
                setNewItem({
                    owner_address: responseData.owner_address,
                    ethAmount: responseData.ethAmount,
                    tokenAmount: responseData.tokenAmount,
